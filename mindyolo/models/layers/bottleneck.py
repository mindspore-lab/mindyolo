...
class AAttn(nn.Cell):
    ...
    def construct(self, x):
        ...
        q, k, v = ops.transpose(qkv.view(B, N, self.num_heads, self.head_dim * 3), (0, 2, 3, 1)).split([self.head_dim, self.head_dim, self.head_dim], dim=2)
        attn = (
...
class ABlock(nn.Cell):
    ...
    def _init_weights(self, m):
        # Initialize weights using a truncated normal distributino
...
class A2C2f(nn.Cell):
    ...
    def construct(self, x):
        ...
        if self.gamma is not None:
            return x + self.gamma.view(1, -1, 1, 1) * self.cv2(ops.cat(y, 1))
        return self.cv2(ops.cat(y, 1))