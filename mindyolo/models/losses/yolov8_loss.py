...
class YOLOv8Loss(nn.Cell):
    ...
    def construct(self, feats, targets, imgs):
        ...
        _, target_bboxes, target_scores, fg_mask, _ = self.assigner(
            self.sigmoid(pred_scores), (pred_bboxes * stride_tensor).astype(gt_bboxes.dtype),
            anchor_points * stride_tensor, gt_labels, gt_bboxes, mask_gt)
        # stop gradient
        target_bboxes, target_scores, fg_mask = \
            ops.stop_gradient(target_bboxes), ops.stop_gradient(target_scores), ops.stop_gradient(fg_mask)
        ...
        return loss.sum() * batch_size, ops.stop_gradient(ops.concat((loss.sum(keepdims=True), loss)))  # loss(box, cls, dfl)
...
class BboxLoss(nn.Cell):
    ...
    def _df_loss(pred_dist, target):
        ...
        loss = (ops.cross_entropy(pred_dist, tl.view(-1), reduction="none").view(tl.shape) * wl +
                ops.cross_entropy(pred_dist, tr.view(-1), reduction="none").view(tl.shape) * wr).mean(-1, keep_dims=True)
...
class TaskAlignedAssigner(nn.Cell):
    ...
    def get_pos_mask(self, pd_scores, pd_bboxes, gt_labels, gt_bboxes, anc_points, mask_gt):
        ...
        mask_topk = self.select_topk_candidates(align_metric * mask_in_gts,
                                                topk_mask=ops.cast(ops.tile(mask_gt[..., None], (1, 1, self.topk)), ms.bool_)
                                                ) # (b, n_gt, h*w)
    ...
    def select_topk_candidates(self, metrics, topk_mask=None):
        ...
        is_in_topk = ops.one_hot(topk_idxs, num_anchors, ops.ones(1, ms.float32), ops.zeros(1, ms.float32)).sum(-2) # (b, n_gt, topk, N) -> (b, n_gt, N)
    ...
    def get_targets(self, gt_labels, gt_bboxes, target_gt_idx, fg_mask):
        ...
        target_scores = ops.one_hot(target_labels, self.num_classes,
                                    on_value=ops.ones(1, ms.int32),
                                    off_value=ops.zeros(1, ms.int32)) # (b, N, 80)
    ...
    def select_highest_overlaps(mask_pos, overlaps, n_gt):
        ...
        is_max_overlaps = ops.one_hot(max_overlaps_idx, n_gt,
                                      on_value=ops.ones(1, ms.int32),
