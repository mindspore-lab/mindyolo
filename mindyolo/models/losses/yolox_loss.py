...
from mindyolo.models.losses.iou_loss import bbox_iou, batch_box_iou
from mindyolo.models.registry import register_model
from mindyolo.models.layers.utils import box_cxcywh_to_xyxy, box_xyxy_to_cxcywh, box_scale, box_clip
...
class YOLOXLoss(nn.Cell):
    ...
    def _get_foreground(self, gt_boxes, gt_valid_mask, center_radius=1.5):
        ...
        gt_core_box_xyxy = ops.concat([gt_box_center[:, :, None, :] - center_radius * self.anchor_strides,
    ...
    def construct(self, preds, targets, imgs=None):
        ...
        # process label
        gt_classes = ops.cast(targets[:, :, 1:2].squeeze(-1), mindspore.int32)
        ...
        cls_preds_ = ops.sigmoid(ops.repeat_elements(self.unsqueeze(cls_preds, 1), rep=gt_max, axis=1)) * \
                     ops.sigmoid(
        ...
        punishment_cost = 1000.0 * (1.0 - ops.cast(is_inbox_and_incenter, mindspore.float32))
        cost = ops.cast(cost + punishment_cost, mindspore.float16)
        # dynamic k matching
        ious_in_boxes_matrix = pair_wise_ious  # (batch_size, gt_max, 8400)
        ious_in_boxes_matrix = ops.cast(pre_fg_mask * ious_in_boxes_matrix, mindspore.float16)
        topk_ious, _ = ops.top_k(ious_in_boxes_matrix, self.n_candidate_k, sorted=True)

        dynamic_ks = ops.reduce_sum(topk_ious, 2).astype(mindspore.int32).clip(xmin=1, xmax=total_num_anchors - 1,
        ...
        pos_mask = ops.cast(cost < max_neg_score, mindspore.float32)  # (batch_size, gt_num, 8400)
        ...
        # calculate target ---------------------------------------------------------------------------------------------
        # Cast precision
        pos_mask = ops.cast(pos_mask, mindspore.float16)
        bbox_true = ops.cast(bbox_true, mindspore.float16)
        gt_classes_ = ops.cast(gt_classes_, mindspore.float16)
        ...
        bbox_preds = ops.cast(bbox_preds, mindspore.float32)
        reg_target = ops.cast(reg_target, mindspore.float32)
        obj_preds = ops.cast(obj_preds, mindspore.float32)
        obj_target = ops.cast(obj_target, mindspore.float32)
        cls_preds = ops.cast(cls_preds, mindspore.float32)
        cls_target = ops.cast(cls_target, mindspore.float32)
        loss_l1 = 0.0
        if self.use_l1:
            l1_target = self.get_l1_format(reg_target)
            l1_preds = self.get_l1_format(bbox_preds)
            l1_target = ops.stop_gradient(l1_target)
            l1_target = ops.cast(l1_target, mindspore.float32)
            l1_preds = ops.cast(l1_preds, mindspore.float32)
